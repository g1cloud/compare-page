...
program
  ...
  .option('--out <prefix>', 'Prefix for the output file names', 'compare')
  .action((urlA, urlB, options) => {
    compareHtml(urlA, urlB, options.selector, options.excludeAttrs, options.excludeAttrRegex, options.out);
  });
